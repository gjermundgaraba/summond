...
struct InstalledAppResolver: AppResolver {
  ...
  static func identity(forApplicationPath path: String) -> AppIdentity? {
    let url = URL(fileURLWithPath: (path as NSString).expandingTildeInPath)
    return identity(forApplicationURL: url)
  }

  static func identity(forApplicationURL url: URL) -> AppIdentity? {
...
struct MacOSAppRuntime: AppRuntime {
  private let system: any MacOSAppRuntimeSystem

  init(logger: Logger = Logger()) {
    self.system = LiveMacOSAppRuntimeSystem(logger: logger)
  }
...
private final class WindowAppearanceObserver {
  ...
  private func setupObserver() -> Bool {
    ...
    guard createResult == .success, let observer = unmanagedObserver else {
      logger.debug(
        "[runtime] AXObserverCreate failed for PID \(processID): \(createResult.rawValue)"
      )
      return false
    }
    ...
    for notification in Self.notifications {
      ...
      case .notificationUnsupported:
        logger.debug(
          "[runtime] AX notification unsupported for PID \(processID): \(notification)"
        )
      default:
        logger.debug(
          "[runtime] AXObserverAddNotification failed for PID \(processID): \(notification) (\(result.rawValue))"
        )
      }
    }
  ...
  fileprivate func handleNotification(_ notification: String) {
    logger.debug("[runtime] received AX notification for PID \(processID): \(notification)")

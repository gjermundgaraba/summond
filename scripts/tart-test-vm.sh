...
vm="${1:-}"
guest_repo="/Volumes/My Shared Files/keybindd"
ready_timeout=300
started_vm=0
run_pid=""
...
wait_for_repo() {
  ...
  until tart exec "$vm" /bin/test -f "$guest_repo/Makefile" >/dev/null 2>&1; do
    if [[ "$started_vm" == "1" ]] && ! kill -0 "$run_pid" >/dev/null 2>&1; then
      wait "$run_pid" || true
      echo "Tart VM '$vm' exited before the keybindd checkout was available." >&2
      exit 1
    fi
    if [[ "$started_vm" != "1" ]] && tart exec "$vm" /usr/bin/true >/dev/null 2>&1; then
      echo "Tart VM '$vm' is already running without the keybindd checkout mounted." >&2
...
if tart exec "$vm" /bin/test -f "$guest_repo/Makefile" >/dev/null 2>&1; then
  ...
  echo "Waiting for already-running Tart VM '$vm' to expose the keybindd checkout..."
  wait_for_repo
else
  echo "Starting Tart VM '$vm' with the keybindd checkout mounted..."
  tart run --dir "keybindd:$ROOT_DIR" "$vm" &
  run_pid=$!
  started_vm=1
  wait_for_repo
fi
...
tart exec "$vm" /bin/zsh -lc \
  'set -euo pipefail
   run_root="$(mktemp -d "${TMPDIR:-/tmp}/keybindd-test.XXXXXX")"
